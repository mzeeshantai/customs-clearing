<x-admin-layout>
    <div class="space-y-6" x-data="expenseEditForm()">
        ...
        <div class="flex items-center gap-4 mb-6">
            ...
            <div>
                <h1 class="text-2xl font-bold text-[#0f172a]">Edit Expense</h1>
                <p class="text-[#64748b] text-[14px] mt-1">Update details for EXP-{{ str_pad($expense->id, 4, '0', STR_PAD_LEFT) }}</p>
            </div>
        </div>
        ...
        <div class="card-c max-w-4xl">
            <form action="{{ route('expenses.update', $expense) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="form-group-c">
                                ...
                                <select name="category_name" class="form-input-c" required x-model="categoryName" @change="onCategoryChange()">
                                    ...
                                    @foreach($categories as $category)
                                        <option value="{{ $category->name }}" {{ old('category_name', $expense->category->name ?? '') == $category->name ? 'selected' : '' }}>{{ $category->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group-c">
                                ...
                                <input type="date" name="expense_date" value="{{ old('expense_date', $expense->expense_date->format('Y-m-d')) }}" class="form-input-c" required>
                    ...
                    <div x-show="isSalary" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            <div class="form-group-c">
                                ...
                                <select name="employee_id" class="form-input-c" x-model="employeeId" @change="fetchEmployee()">
                                    ...
                                    @foreach($employees as $employee)
                                        <option value="{{ $employee->id }}" {{ old('employee_id', $expense->employee_id) == $employee->id ? 'selected' : '' }}>{{ $employee->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group-c">
                                ...
                                <input type="date" name="salary_month" value="{{ old('salary_month', $expense->salary_month ? $expense->salary_month->format('Y-m-d') : '') }}" class="form-input-c" x-bind:required="isSalary">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="form-group-c">
                                ...
                                <input type="number" step="0.01" name="bonus" x-model="bonus" @input="calcFinal()" class="form-input-c" placeholder="0.00">
                                <x-input-error :messages="$errors->get('bonus')" class="mt-2" />
                            </div>
                            <div class="form-group-c">
                                <label class="form-label-c">Deduction (Optional)</label>
                                <input type="number" step="0.01" name="deduction" x-model="deductionAmt" @input="calcFinal()" class="form-input-c" placeholder="0.00">
                    ...
                    <div x-show="!isSalary" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="form-group-c">
                                ...
                                <input type="text" name="title" value="{{ old('title', $expense->title) }}" class="form-input-c" x-bind:required="!isSalary">
                                <x-input-error :messages="$errors->get('title')" class="mt-2" />
                            </div>
                            <div class="form-group-c">
                                <label class="form-label-c">Amount (Rs.) <span class="text-red-500">*</span></label>
                                <input type="number" step="0.01" name="amount" value="{{ old('amount', $expense->amount) }}" class="form-input-c" placeholder="0.00" x-bind:required="!isSalary">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div class="form-group-c">
                                ...
                                <input type="text" name="reference_no" value="{{ old('reference_no', $expense->reference_no) }}" class="form-input-c" placeholder="Optional">
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            <div class="form-group-c" x-show="!isSalary">
                                ...
                                @if($expense->attachment_path)
                                    <div class="mb-3 flex items-center gap-3 p-3 bg-[#f8fafc] border border-[#e5e9f2] rounded-xl w-fit">
                                        <i class="bi bi-file-earmark-text text-[#1565c0] text-xl"></i>
                                        <a href="{{ Storage::url($expense->attachment_path) }}" target="_blank" class="text-sm font-medium text-[#1565c0] hover:underline">View Current Attachment</a>
                                        
                                        <label class="ml-4 flex items-center gap-2 cursor-pointer">
                                            <input type="checkbox" name="remove_attachment" value="1" class="rounded border-gray-300 text-red-600 focus:ring-red-600">
                                            <span class="text-xs text-red-600 font-bold">Remove</span>
                                        </label>
                                    </div>
                                @endif
                                ...
                                <p class="text-xs text-gray-500 mt-1">Upload a new file to replace the current one. Max size: 5MB.</p>
                            </div>

                            <div class="form-group-c">
                                <label class="form-label-c">Notes / Description</label>
                                <textarea name="notes" rows="3" class="form-input-c" placeholder="Optional description...">{{ old('notes', $expense->notes) }}</textarea>
                ...
                <div class="px-6 py-4 md:px-8 bg-[#f8fafc] border-t border-[#e5e9f2] flex items-center justify-end gap-3">
                    ...
                    <button type="submit" class="btn-brand px-8 py-2.5">Update Expense</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        function expenseEditForm() {
            return {
                categoryName: '{{ old("category_name", $expense->category->name ?? "") }}',
                isSalary: false,
                employeeId: '{{ old("employee_id", $expense->employee_id ?? "") }}',
                empName: '{{ $expense->employee->name ?? "" }}',
                empDesignation: '{{ $expense->employee->designation ?? "" }}',
                empSalary: {{ $expense->employee->monthly_salary ?? $expense->amount ?? 0 }},
                empLoaded: {{ $expense->employee_id ? 'true' : 'false' }},
                bonus: '{{ old("bonus", $expense->bonus ?? "") }}',
                deductionAmt: '{{ old("deduction", $expense->deduction ?? "") }}',
                finalAmount: {{ $expense->final_amount ?? $expense->amount ?? 0 }},
