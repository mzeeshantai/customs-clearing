<x-admin-layout>
    <x-slot name="header">
        Edit Bill: <span class="text-indigo-600">{{ $bill->bill_no }}</span>
    </x-slot>

    <div class="max-w-7xl mx-auto space-y-6">
        <form action="{{ route('bills.update', $bill) }}" method="POST" x-data="billForm()">
            @csrf
            @method('PUT')
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="bg-white rounded shadow-md border-t-4 border-indigo-600 overflow-hidden">
                        ...
                        <div class="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            <div class="space-y-1">
                                <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider">Bill No <span class="text-rose-500">*</span></label>
                                <input type="text" name="bill_no" value="{{ old('bill_no', $bill->bill_no) }}" required class="w-full text-sm border-slate-200 rounded focus:ring-indigo-500 focus:border-indigo-500 font-bold text-indigo-700 bg-indigo-50/50">
                            </div>
                            <div class="space-y-1">
                                <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider">Date <span class="text-rose-500">*</span></label>
                                <input type="date" name="date" value="{{ old('date', $bill->date) }}" required class="w-full text-sm border-slate-200 rounded focus:ring-indigo-500 focus:border-indigo-500">
                            </div>
                            <div class="space-y-1">
                                <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider">Payment Status <span class="text-rose-500">*</span></label>
                            ...
                            <div x-show="paymentStatus === 'paid'" x-transition class="space-y-1">
                                <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider">Payment Method <span class="text-rose-500">*</span></label>
                                <select name="payment_method" :required="paymentStatus === 'paid'" class="w-full text-sm border-slate-200 rounded focus:ring-indigo-500 focus:border-indigo-500">
                                    ...
                                    <option value="pay_order" {{ old('payment_method', $bill->payment_method) == 'pay_order' ? 'selected' : '' }}>Pay Order</option>
                                    <option value="cheque" {{ old('payment_method', $bill->payment_method) == 'cheque' ? 'selected' : '' }}>Cheque</option>
                                    <option value="online_transfer" {{ old('payment_method', $bill->payment_method) == 'online_transfer' ? 'selected' : '' }}>Online Transfer</option>
                            ...
                            <div class="lg:col-span-1 md:col-span-2">
                                ...
                                <select name="client_id" x-model="selectedClientId" @change="updateClientInfo" required class="w-full text-sm border-slate-200 rounded focus:ring-indigo-500 focus:border-indigo-500 font-bold text-slate-700">
                    ...
                    <div class="bg-white rounded shadow-md border-t-4 border-emerald-500 overflow-hidden">
                        ...
                        <div class="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            <div>
                                ...
                                <input type="text" name="gd_no" value="{{ $bill->gd_no }}" required class="w-full text-sm border-slate-200 rounded focus:ring-emerald-500 focus:border-emerald-500">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-slate-500 uppercase mb-1">GD Date</label>
                                <input type="date" name="gd_date" required value="{{ $bill->gd_date }}" class="w-full text-sm border-slate-200 rounded focus:ring-emerald-500 focus:border-emerald-500">
                            </div>
                            <div>
                    ...
                    <div class="bg-white rounded shadow-md border-t-4 border-indigo-600 overflow-hidden sticky top-20">
                        ...
                        <div class="p-6 space-y-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="space-y-1">
                                    <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider">Marks & Nos</label>
                                    <textarea name="marks_and_nos" rows="2" class="w-full text-sm border-slate-200 rounded focus:ring-indigo-500 focus:border-indigo-500 placeholder:text-slate-300" placeholder="Optional marks...">{{ old('marks_and_nos', $bill->marks_and_nos) }}</textarea>
                                </div>
                                <div class="space-y-1">
                                    <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider">Remarks / Notes</label>
                                    <textarea name="remarks" rows="2" class="w-full text-sm border-slate-200 rounded focus:ring-indigo-500 focus:border-indigo-500 placeholder:text-slate-300" placeholder="Any internal notes or special instructions...">{{ old('remarks', $bill->remarks) }}</textarea>
                                </div>
                            </div>
    ...
    <script>
        function billForm() {
            ...
            const billItemsData = @json($bill->items);

            return {
                paymentStatus: billData.status || 'unpaid',
                selectedClientId: billData.client_id || '',
                clientNtn: '',
                clientAddress: '',
                ...
                items: billItemsData.map(i => ({
                    ...
                    pay_order_amount: parseFloat(i.pay_order_amount) || 0,
                    is_paid_by_agent: i.is_paid_by_agent == 1,
                    short_payment: parseFloat(i.short_payment) || 0,
                    is_fixed: ['custom duty and other taxes', 'token for gd filling', 'kict/aict/qict/kpt wharfage', 'demurrage', 'shipping charges', 'excise & taxation fee', 'stamp duty', 'weighment charges', 'other misc expences', 'additional / short duty', 'addition / short excise', 'terminal short payment', 'shipping co short payment'].includes(i.particular_name.toLowerCase())
                })),

                init() {
                    // Force the selected state correctly on initial load
                    this.$nextTick(() => {
                        const select = document.querySelector('select[name="client_id"]');
                        if (select) {
                            this.updateClientInfo({ target: select });
                        }
                    });
                },

                updateClientInfo(e) {
                    const select = e ? e.target : event.target;
